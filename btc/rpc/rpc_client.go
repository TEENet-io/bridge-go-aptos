...
// Create a new RPC client with limited functions
...
// Enable -txindex on your node before using this function.
...
// Notice: You fill in either P2PKH or P2WPKH, the result is specific to that address type.
func (r *RpcClient) GetUtxoList(myAddress btcutil.Address) ([]data.UTXO, error) {
	...
	for _, item := range unspentOutputs {
		...
		var pkType data.PubKeyScriptType
		if txscript.IsPayToPubKeyHash(outputPoint.PkScript) {
			pkType = data.P2PKH_SCRIPT_T
		}
		if txscript.IsPayToWitnessPubKeyHash(outputPoint.PkScript) {
			pkType = data.P2WPKH_SCRIPT_T
		}

		u = append(u, data.UTXO{TxID: item.TxID, TxHash: txRaw.Hash(), Vout: item.Vout, Amount: outputPoint.Value, PkScriptT: pkType, PkScript: outputPoint.PkScript})
	}
	return u, nil
}

func (r *RpcClient) SendRawTx(tx *wire.MsgTx) (*chainhash.Hash, error) {
	// allowHighFees is a protection
	// if bitcoin node thinks your fee is too high (maybe due to mistakes) it can reject you.
