...
var (
	...
	MonitoredTxTable = `CREATE TABLE IF NOT EXISTS MonitoredTx (
		...
		CONSTRAINT chk_rx CHECK (rx != '` + strZeroBytes32 + `'),
		CONSTRAINT chk_s CHECK (s != '` + strZeroBytes32 + `'),
	...
	queryInsertPendingMonitoredTx = `INSERT INTO MonitoredTx (
		txHash, id, signingHash, outpoints, rx, s, sentAfter, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8);`
	queryInsertMonitoredTx = `INSERT INTO MonitoredTx (
		txHash, id, signingHash, outpoints, rx, s, sentAfter, minedAt, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);`
