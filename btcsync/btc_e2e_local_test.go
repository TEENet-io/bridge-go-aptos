package btcsync_test

// Notice:
// This test uses a local bitcoin two-node network (regtest mode) v0.21.
// And a Ethereum local Geth network (regtest mode) v1.14.
...
func setupBtcMonitor(t *testing.T, r *rpc.RpcClient, st btcaction.RedeemActionStorage, startBlock int) (*btcsync.BTCMonitor, error) {
...
func TestDeposit(t *testing.T) {
	...
	mint_observer := btcsync.NewBTC2EVMObserver(ethEnv.st, CHANNEL_BUFFER_SIZE)
	...
	utxo_observer := btcsync.NewObserverUTXOVault(my_btc_vault, CHANNEL_BUFFER_SIZE)
